...
R = jnp.asarray([[1.0]], dtype=jnp.float32)
DT = 0.05
HORIZON = 25
...
class ObjectiveWithControlCost(BaseObjective):
    def __init__(self, terminal_cost):
        super().__init__()
        self.terminal_cost = terminal_cost
    ...
    def running_cost(self, state, inputs, reference):
        err = state - reference
        control_cost = inputs[0] * inputs[0]
        return jnp.asarray(
            20.0 * (err.T @ Q @ err) + control_cost,
            dtype=jnp.float32,
        )
...
def lqr_seed_with_control_cost():
    ad = jnp.array([[1.0, DT], [0.0, 1.0]], dtype=jnp.float32)
    bd = jnp.array([[DT * DT], [DT]], dtype=jnp.float32)
    k, s, _ = control.dlqr(ad, bd, Q, R)
    k = jnp.asarray(k, dtype=jnp.float32)
    s = jnp.asarray(s, dtype=jnp.float32)
...
def build_local_lqr_solver(terminal_cost):
    robot_config = settings.RobotConfig()
    robot_config.nq = 1
    robot_config.nv = 1
    robot_config.nu = 2
    ...
    config.MPC.num_control_points = config.MPC.horizon
    config.MPC.gains = True
    config.MPC.gain_method = "local_lqr"
    config.solver_dynamics = settings.DynamicsModel.CUSTOM
    config.sim_dynamics = settings.DynamicsModel.CUSTOM

    _, solver = build_model_and_solver(
        config, ObjectiveWithControlCost(terminal_cost), custom_dynamics_fn=dynamics
    )
    return solver
...
def test_local_lqr_gain_matches_expected_lqr_feedback_for_quadratic_cost():
    seed, terminal_cost, lqr_gain = lqr_seed_with_control_cost()
    solver = build_local_lqr_solver(terminal_cost)
    solver.sampler.optimal_samples = seed
    solver.command(
        jnp.array([0.0, 0.0], dtype=jnp.float32),
        jnp.array([0.5, 0.0], dtype=jnp.float32),
    ...
    local_lqr = jax.block_until_ready(solver.gains)

    assert local_lqr.shape == (2, 2)
    assert jnp.all(jnp.isfinite(local_lqr))
    assert jnp.linalg.norm(local_lqr[1], ord=jnp.inf) < 1e-6

    ad = jnp.array([[1.0, DT], [0.0, 1.0]], dtype=jnp.float32)
    bd = jnp.array([[DT * DT], [DT]], dtype=jnp.float32)
    open_loop_radius = jnp.max(jnp.abs(jnp.linalg.eigvals(ad)))
    closed_loop_radius = jnp.max(
        jnp.abs(jnp.linalg.eigvals(ad + bd @ local_lqr[:1, :]))
    )

    assert closed_loop_radius < open_loop_radius
    assert closed_loop_radius < 1.0
